...
- 需要写可运行代码时，当前 Custom 模型会自动走双 Agent：先 plan 再写代码，再由独立 Agent 审查并在本机受限环境跑测试；最多 3 轮。只有测试真正跑起来并且发现代码有 bug 才会回到写代码 Agent；测试命令本身无效不会拿去空转。HTML/JS 页面用静态完整性检查，不再强行跑 Python unittest。搜索/读取仍共用单次回答最多 3+3 次的额度，非写代码任务保持原流程
