...
- 需要写可运行代码时，当前 Custom 模型会自动走双 Agent：先 plan 再写代码，再由独立 Agent 审查并在本机受限环境跑测试；最多 2 轮，到上限就停止并交出当前结果。HTML/JS 页面用静态完整性检查，不再强行跑 Python unittest。搜索/读取仍共用单次回答最多 3+3 次的额度，非写代码任务保持原流程
