...
function isOpenCodeDsmlTarget(provider=selectedProvider(),model=selectedModel()){if(!provider||normalizeProviderType(provider.provider_type)!=='custom'||String(model||'').trim().toLowerCase().split('/').pop()!=='deepseek-v4-flash-free')return false;try{return new URL(provider.base_url).hostname.toLowerCase()==='opencode.ai'}catch{return false}}
function hasOpenCodeDsmlTarget(provider){return providerModels(provider).some(model=>isOpenCodeDsmlTarget(provider,model))}
function fillCustomSettings(){
  const provider=selectedProvider(),config=customSettings(provider),dsmlTarget=isOpenCodeDsmlTarget(provider),dsmlToggle=$('#customDsmlFallbackEnabled');
  $('#customThinking').value=config.thinking;$('#customReasoningEffortEnabled').checked=config.reasoning_effort_enabled;$('#customIncludeReasoningEnabled').checked=config.include_reasoning_enabled;dsmlToggle.disabled=!dsmlTarget;dsmlToggle.checked=dsmlTarget&&config.dsml_fallback_enabled;$('#customMaxCompletion').value=config.max_completion_tokens;$('#customTemperature').value=config.temperature;$('#customTopP').value=config.top_p;$('#customWebToolBackend').value=config.web_tool_backend;
...
async function saveCustomSettings(event){
  ...
  const config=customSettings(provider),dsmlTarget=isOpenCodeDsmlTarget(provider);const body={thinking:$('#customThinking').value,reasoning_effort_enabled:$('#customReasoningEffortEnabled').checked,include_reasoning_enabled:$('#customIncludeReasoningEnabled').checked,dsml_fallback_enabled:dsmlTarget?$('#customDsmlFallbackEnabled').checked:(hasOpenCodeDsmlTarget(provider)&&config.dsml_fallback_enabled),max_completion_tokens:Number($('#customMaxCompletion').value),temperature:Number($('#customTemperature').value),top_p:Number($('#customTopP').value),web_tool_backend:$('#customWebToolBackend').value};
