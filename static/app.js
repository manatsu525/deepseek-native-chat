...
function storageKey(name) { return `custom-native-chat.${name}.${state.me ? state.me.id : 'guest'}`; }
...
function syncManagementUi(){
  const admin=isAdmin();
  $('#providerAccessNote').textContent=admin
    ?'API、模型和 Custom 参数是全局共享配置；当前管理员可以统一维护。'
    :'API、模型和 Custom 参数由管理员统一维护；当前账号只能使用共享配置。';
  $('#providerForm').classList.toggle('hidden',!admin);
  $('#skillAccessNote').textContent=admin
    ?'Skill 的安装、启用和删除会对所有账号的 Agent 模式生效。'
    :'Skill 内容和启用状态是全局共享的；当前账号只能查看和使用，请联系管理员修改。';
  $('#skillInstallForm').classList.toggle('hidden',!admin);
}
function renderSkills(){
  const skills=Array.isArray(state.skills)?state.skills:[],enabled=skills.filter(item=>item.enabled).length,admin=isAdmin();
  ...
  $('#skillList').innerHTML=skills.length?skills.map(skill=>{
    ...
    const mutations=admin?`<button class="${toggleClass}" type="button" data-skill-toggle="${id}" data-skill-enabled="${skill.enabled?'true':'false'}">${toggle}</button>${skill.builtin?'':`<button class="danger-btn" type="button" data-skill-delete="${id}">删除</button>`}`:'';
    return `<article class="skill-item"><div class="skill-item-main"><strong>${name}${badge}</strong><small>${description}</small></div><div class="skill-item-actions"><button class="soft-btn" type="button" data-skill-read="${id}">查看</button>${mutations}</div></article>`;
...
async function loadSkills(showError=false){
  try{
    const data=await api('/api/skills');state.skills=data.skills||[];syncManagementUi();renderSkills();
  }catch(err){
...
async function toggleSkill(button){
  if(!isAdmin()){toast('仅管理员可修改共享 Skill');return}
...
async function openSkills(){
  $('#skillStatus').textContent='';clearSkillDetail();syncManagementUi();$('#skillsModal').showModal();await loadSkills(true);
}
...
function traceHtml(meta={}, active=false, detailKey='trace') {
  ...
  const searchHtml = searches.map((s,i) => {
    ...
    const label=s.action==='workspace'?(workspaceLabels[s.tool]||'工作区操作'):s.action==='agent'?(agentLabels[s.tool]||s.tool||'Agent 操作'):s.action==='plan'?'更新计划':s.action==='tools'?'加载工具':s.action==='open_page'?'读取网页':'联网搜索';
    const searchKey=`${detailKey}-search-${s.id || i}`;
    const searchOpen=detailState.get(searchKey) ? ' open' : '';
    const fallbackDetail=s.action==='search'?(s.status==='running'||s.status==='searching'?'准备查询词':'未返回查询词'):s.action==='open_page'?'未返回网页地址':'工具未返回详情';
    const detail=s.action==='workspace'||s.action==='agent'||s.action==='tools'||s.action==='plan'?escapeHtml(s.path||s.query||(s.action==='agent'?'Agent 操作':s.action==='tools'?'加载工具':'工作区')):s.url?`<a href="${safeUrl(s.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(s.url)}</a>`:escapeHtml(Array.isArray(s.query)?s.query.filter(x=>!String(x).startsWith('ws_call_id=')).join('；'):(s.query||fallbackDetail));
...
function messageHtml(message, index, live=false, retryable=false) {
  ...
  const assistantName = agent ? 'Agent' : 'Custom';
  return `<article class="message ${assistant ? 'assistant' : 'user'}${live ? ' live-message' : ''}" data-index="${index}">
    <div class="message-icon">${assistant ? (agent ? 'AG' : 'CU') : escapeHtml(((state.me && state.me.username) || 'U')[0].toUpperCase())}</div>
...
function normalizeProviderType(value){return value==='mimo'?'custom':value==='deepseek'?'custom_response':(value||'custom')}
...
function providerLabel(provider){const kind=normalizeProviderType(provider.provider_type);return kind==='custom_response'?'Custom Responses':kind==='custom_messages'?'Custom Messages':'Custom Chat'}
...
function updateProviderUi(){
  ...
  $('#customSettingsButton').classList.toggle('hidden',!custom||!isAdmin());
  $('#effortControl').classList.toggle('hidden',!!custom);
  $('#effort').disabled=!!custom;
  $('#effort').title='思考档位请在“Custom 参数”中设置';
  $('#nativePill').textContent=agent?'● Agent · 全权限':custom?`● ${webInfo.label}`:'● Custom';
  $('#welcomeOrbitMark').textContent=agent?'AG':'CU';
  $('#welcomeEyebrow').textContent=agent?'HOST AGENT · SKILLS':responses?'CUSTOM · RESPONSES':messages?'CUSTOM · MESSAGES':'CUSTOM · CHAT COMPLETIONS';
  $('#welcomeTitle').textContent=agent?'让 Agent 直接管理代码与服务器':'使用 Custom 本地联网';
  $('#welcomeDescription').textContent=agent?'Agent 直接使用主机文件、Shell、对话、Skill 和前端管理工具。':`模型通过标准 ${responses?'Responses':messages?'Anthropic Messages':'Chat Completions'} 协议调用 ${webInfo ? webInfo.label : '联网工具'} 搜索与读取真实来源。`;
  if($('#statusText'))$('#statusText').textContent=agent?(custom?'Agent 模式 · 主机工具已就绪':'Agent 模式 · 等待 Custom 模型'):'标准模式 · 外部搜索工具已就绪';
  if($('#footnote'))$('#footnote').textContent=agent?'Agent 模式拥有主机级文件和命令权限，并内置编程、对话、Skill、前端管理能力；工作区固定为 /home/share，与普通模式严格分开；每轮最多执行一次联网工具，其他工具按顺序执行。':'标准模式：Custom API 使用你选择的搜索与网页抓取方案。';
...
async function loadProviders(){
  ...
  syncManagementUi(); renderProviderList(); updateProviderUi();
}
function renderProviderList(){
  const actions=isAdmin();
  $('#providerList').innerHTML=state.providers.map(p=>`<div class="list-item"><div class="list-item-main"><strong>${escapeHtml(p.name)} <span class="provider-kind">${providerLabel(p)}</span></strong><small>${escapeHtml(p.api_key_masked)} · ${escapeHtml(providerModels(p).join('、'))}</small></div><div class="item-actions">${actions?`<button class="soft-btn" data-provider-edit="${p.id}">编辑 API</button><button class="danger-btn" data-provider-delete="${p.id}">删除</button>`:'<span class="muted">共享配置 · 只读</span>'}</div></div>`).join('')||'<p class="muted">尚未添加 API。</p>';
...
function syncProviderForm(){
  ...
  if(!state.editingProviderId)base.value=providerType()==='custom_messages'?'https://api.anthropic.com/v1':'https://api.openai.com/v1';
  model.innerHTML='<option value=""></option>';
  $('#providerModelField').classList.add('hidden');
  $('#manualModelField').classList.remove('hidden');
  $('#customModelsPanel').classList.toggle('hidden',!custom||!$('#customModelList input'));
  $('#providerName').placeholder='例如：我的 Custom API';
...
function resetProviderEditor(){
  ...
  $('#providerType').value='custom';syncProviderForm();$('#providerKey').value='';$('#manualModel').value='';renderCustomModels([],[]);$('#providerStatus').textContent='';
}
async function editProviderModels(providerId){
  if(!isAdmin()){toast('仅管理员可修改共享 API');return}
...
async function testProvider(){
  if(!isAdmin()){toast('仅管理员可测试或修改共享 API');return}
  const button=$('#testProvider');button.disabled=true;$('#providerStatus').textContent='正在连接 Custom…';
  try{
...
function syncCustomThinkingFields(){const protocol=normalizeProviderType((selectedProvider()||{}).provider_type),responses=protocol==='custom_response',messages=protocol==='custom_messages',mimo=isMimoModel(selectedModel()),thinking=$('#customThinking').value==='enabled',effortEnabled=$('#customReasoningEffortEnabled').checked,samplingBlocked=messages?thinking:!responses&&mimo&&thinking;$('#customThinking').disabled=responses;$('#customReasoningEffort').disabled=!effortEnabled;$('#customReasoningEffortEnabled').disabled=false;$('#customTemperatureEnabled').disabled=samplingBlocked;$('#customTemperature').disabled=samplingBlocked;$('#customTopPEnabled').disabled=samplingBlocked;$('#customTopP').disabled=samplingBlocked;$('#customSamplingNote').textContent=samplingBlocked?'当前 thinking 模式不发送 temperature/top_p；如需手动加入，请使用高级 JSON。':`默认不发送 temperature/top_p；勾选对应开关后才会加入真实上游 JSON。${responses?' Responses 协议使用官方 reasoning.effort。':''}`}
...
$('#providerButton').onclick=()=>{syncManagementUi();resetProviderEditor();$('#providerModal').showModal();renderProviderList()};$('#skillsButton').onclick=openSkills;$('#skillsRefresh').onclick=()=>loadSkills(true);$('#skillDetailClose').onclick=clearSkillDetail;$('#usersButton').onclick=()=>{loadUsers();$('#usersModal').showModal()};
