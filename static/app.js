...
function customSettings(provider,model=selectedModel()){
  const defaults={thinking:'enabled',reasoning_effort:'high',reasoning_effort_enabled:true,lowest_price_aggregators:[],dsml_fallback_enabled:false,text_replace_tool:false,max_completion_tokens:65536,temperature:1,top_p:.95,web_tool_backend:'parallel',request_overrides:{},advanced_enabled:false,advanced_request:{}};
...
async function fillCustomSettings(){
  ...
  $('#customThinking').value=config.thinking;$('#customReasoningEffort').value=config.reasoning_effort||'high';$('#customReasoningEffortEnabled').checked=config.reasoning_effort_enabled;$('#customLowestPriceOpenRouter').checked=aggregators.has('openrouter');$('#customLowestPriceVercel').checked=aggregators.has('vercel');$('#customDsmlFallbackEnabled').checked=config.dsml_fallback_enabled;$('#customTextReplaceTool').checked=Boolean(config.text_replace_tool);$('#customMaxCompletion').value=config.max_completion_tokens;$('#customTemperature').value=config.temperature;$('#customTopP').value=config.top_p;$('#customWebToolBackend').value=config.web_tool_backend;
