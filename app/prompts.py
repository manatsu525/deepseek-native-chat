...
- Verify with the smallest relevant check: use check_web_syntax for HTML/JS and a targeted unittest module or focused command for the files changed. Do not run the entire test-discovery suite after every edit; run it once for cross-cutting runtime, tool or schema changes, or before final delivery, and never rerun an unchanged passing test. Treat ok=false or a nonzero exit as a real failure. Syntax success does not prove runtime behavior; say so.
