...
class OpenCodeHeaderTests(unittest.TestCase):
    def test_opencode_hosts_receive_requested_user_agent_and_session(self) -> None:
        for base_url in ("https://opencode.ai/zen/v1", "https://api.opencode.ai/v1"):
            ...
            chat_headers = custom_auth_headers(
                "secret",
                base_url=base_url,
                stream=True,
                conversation_id="chat-session-123",
            )
            self.assertEqual(chat_headers["x-opencode-session"], "chat-session-123")
    ...
    def test_other_and_lookalike_hosts_are_unchanged(self) -> None:
        for base_url in ("https://api.openai.com/v1", "https://opencode.ai.example.com/v1", ""):
            ...
            headers = custom_auth_headers("secret", base_url=base_url, conversation_id="chat-session-123")
